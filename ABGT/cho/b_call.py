...
def call_router(client:redis.Redis):
    ...
    @router_call.callback_query(F.data.startswith('prof_list_ru_'))
    ...
    async def us_pur(callback:types.CallbackQuery):
        ...
        index_int = int(index_str)
        async with Session() as session:
            fi_u_pu = await session.scalar(select(UserPurchases).where(UserPurchases.user_id == u).order_by(UserPurchases.user_purchases.asc()).offset(index_int).limit(1))
            use_pur = fi_u_pu.user_purchases
        gi_anim = await gift_info(use_pur[f'gift_id{index_int}'])
        await callback.message.delete()
        await callback.message.answer(langs[ul]['BGI'].format(stick=gi_anim.sticker.emoji,pr=gi_anim.star_count,time=use_pur[f'gift_time{index_int}']),reply_markup=keybos[ul]['B'])
